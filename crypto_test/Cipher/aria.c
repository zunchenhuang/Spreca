...
void aria_encrypt(const unsigned char *in, unsigned char *out,
                  const ARIA_KEY *key __attribute__((annotate("mark")))) __attribute__((always_inline))
{
    register uint32_t reg0, reg1, reg2, reg3;
    int Nr;
    const ARIA_u128 *rk __attribute__((annotate("mark")));
...
int aria_set_encrypt_key(const unsigned char *userKey, const int bits,
                         ARIA_KEY *key) __attribute__((always_inline))
{
    register uint32_t reg0, reg1, reg2, reg3;
 ...
 */
static void xor128(ARIA_c128 o, const ARIA_c128 x, const ARIA_u128 *y) __attribute__((always_inline))
{
    int i;
...
static ossl_inline void rotnr(unsigned int n, ARIA_u128 *o,
                              const ARIA_u128 *xor, const ARIA_u128 *z) __attribute__((always_inline))
{
 ...
 * It is safe for the output to overlap either input.
 */
static void rot19r(ARIA_u128 *o, const ARIA_u128 *xor, const ARIA_u128 *z) __attribute__((always_inline))
{
    rotnr(19, o, xor, z);
}
 ...
 * It is safe for the output to overlap either input.
 */
static void rot31r(ARIA_u128 *o, const ARIA_u128 *xor, const ARIA_u128 *z) __attribute__((always_inline))
{
    rotnr(31, o, xor, z);
}
 ...
 * It is safe for the output to overlap either input.
 */
static void rot61l(ARIA_u128 *o, const ARIA_u128 *xor, const ARIA_u128 *z) __attribute__((always_inline))
{
 ...
 * It is safe for the output to overlap either input.
 */
static void rot19l(ARIA_u128 *o, const ARIA_u128 *xor, const ARIA_u128 *z) __attribute__((always_inline))
{
 ...
 */
static void sl1(ARIA_u128 *o, const ARIA_u128 *x, const ARIA_u128 *y) __attribute__((always_inline))
{
    unsigned int i;
 ...
 * It is safe for the input and output to be the same.
 */
static void sl2(ARIA_c128 o, const ARIA_u128 *x, const ARIA_u128 *y) __attribute__((always_inline))
{
    unsigned int i;
    for (i = 0; i < ARIA_BLOCK_SIZE; i += 4) {
        o[i    ] = sb3[x->c[i	 ] ^ y->c[i    ]];
 ...
 */
static void a(ARIA_u128 *y, const ARIA_u128 *x) __attribute__((always_inline))
{
...
static ossl_inline void FO(ARIA_u128 *o, const ARIA_u128 *d,
                           const ARIA_u128 *rk)  __attribute__((always_inline))
{
    ARIA_u128 y;
...
static void do_encrypt(unsigned char *o, const unsigned char *pin,
                       unsigned int rounds, const ARIA_u128 *keys) __attribute__((always_inline))
{
    ARIA_u128 p;
    unsigned int i;
 ...
 */
void aria_encrypt(const unsigned char *in, unsigned char *out,
                  const ARIA_KEY *key) __attribute__((always_inline))
{
...
int main()
{
    ...
    aria_set_encrypt_key(key, 256, &enc_key);
    ...
    char *buff = malloc(sizeof(char) * 2640);
    for (i = 0; i < 2640; i+= 16) {
      buff[i] = i;
    }

    char *input = malloc(sizeof(char) * size * round);
    for (size_t n = 0; n < length * round; n++) {
        int key = rand() % 94;
        input[n] = charset[key];
    }
    ...
    while(round) {
      /* printf("original:\t"); */
      /* for(i=(round-1)*size;i<round*size;i++) */
      /*   printf("%X ",*(input+i)); */
      /* printf("       "); */
      /* for(i=(round-1)*size;i<round*size;i++) */
      /*   printf("%c",*(input+i)); */
      ...
      char* in = malloc(size);
      strncpy(in, input+(round-1)*size, size);
      aria_encrypt(in, enc_out, &enc_key);    
