...
int Camellia_Ekeygen(int keyBitLength, const u8 *rawKey __attribute__((annotate("mark"))), KEY_TABLE_TYPE k) __attribute__((always_inline))
{
...
void Camellia_EncryptBlock_Rounds(int grandRounds, const u8 plaintext[],
                                  const KEY_TABLE_TYPE keyTable __attribute__((annotate("mark"))),
                                  u8 ciphertext[]) __attribute__((always_inline))
{
    register u32 s0, s1, s2, s3;
    const u32 *k __attribute__((annotate("mark"))) = keyTable, *kend = keyTable + grandRounds * 16;
...
void Camellia_EncryptBlock(int keyBitLength, const u8 plaintext[],
                           const KEY_TABLE_TYPE keyTable __attribute__((annotate("mark"))), u8 ciphertext[]) __attribute__((always_inline))
{
...
void Camellia_DecryptBlock_Rounds(int grandRounds, const u8 ciphertext[],
                                  ...
                                  u8 plaintext[]) __attribute__((always_inline))
{
    u32 s0, s1, s2, s3;
    const u32 *k __attribute__((annotate("mark"))) = keyTable + grandRounds * 16, *kend = keyTable + 4;
...
void Camellia_DecryptBlock(int keyBitLength, const u8 plaintext[],
                           const KEY_TABLE_TYPE keyTable, u8 ciphertext[]) __attribute__((always_inline))
{
...
int main()
{
    const u8 Key[] = "3141592653589793";
    const u8 *rawKey = Key;
    KEY_TABLE_TYPE k;

    u8 ciphertext;
    Camellia_Ekeygen(128, rawKey, k);
    ...
    char *buff = malloc(sizeof(char) * 2880);
    for (i = 0; i < 2880; i+= 64) {
      int key = rand() % length;
      buff[i] = charset[key];
    }
    ...
    while(round) {
      ...
      Camellia_EncryptBlock(128, in, k, &ciphertext);
      free(in);
      round--;
    }
}
