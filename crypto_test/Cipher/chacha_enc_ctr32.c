...
static void chacha20_core(chacha_buf *output, const u32 input[16]) __attribute__((always_inline))
{
    u32 x[16];
    int i;
    const union {
...
void ChaCha20_ctr32(unsigned char *out, const unsigned char *inp,
                    size_t len, const unsigned int key[8] __attribute__((annotate("mark"))),
                    const unsigned int counter[4]) __attribute__((always_inline))
{
    u32 input[16];
    chacha_buf buf ;
    size_t todo, i;
...
int main()
{
    //const u32 input[16] = {3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5, 8, 9, 7, 9, 3};
    const unsigned char *input = "checking chacha string encrypt";
    chacha_buf output;
    const int key[8] = {3, 1, 4, 1, 5, 9, 2, 6};
    const int counter[4] = {0, 0, 0, 0};
    //chacha20_core(&output, input);
    ...
    ChaCha20_ctr32(&output, input, 256, key, counter);
    return 0;
} 
