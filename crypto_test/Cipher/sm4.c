...
static ossl_inline uint32_t rotl(uint32_t a, uint8_t n)  __attribute__((always_inline))
{
    return (a << n) | (a >> (32 - n));
}

static ossl_inline uint32_t load_u32_be(const uint8_t *b, uint32_t n)  __attribute__((always_inline))
{
...
static ossl_inline void store_u32_be(uint32_t v, uint8_t *b)  __attribute__((always_inline))
{
    b[0] = (uint8_t)(v >> 24);
    b[1] = (uint8_t)(v >> 16);
    b[2] = (uint8_t)(v >> 8);
...
static ossl_inline uint32_t SM4_T_slow(uint32_t X)  __attribute__((always_inline))
{
    uint32_t t = 0;
...
static ossl_inline uint32_t SM4_T(uint32_t X)  __attribute__((always_inline))
{
...
int SM4_set_key(const uint8_t *key, SM4_KEY *ks)  __attribute__((always_inline))
{
    /*
     * Family Key
     */
...
void SM4_encrypt(const uint8_t *in, uint8_t *out, const SM4_KEY *ks __attribute__((annotate("mark"))))  __attribute__((always_inline))
{
...
int main()
{
    ...
    char *buff = malloc(sizeof(char) * 5200);
    for (i = 0; i < 5200; i+= 16) {
      buff[i] = i;
    }
    ...
    while(round) {
      ...
      SM4_encrypt(in, enc_out, &enc_key);    
